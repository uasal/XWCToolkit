...
/** Manages a binary file containing XWCToolkit logs.
  *
...
protected:
   ...
   std::string m_logExt {XWCTK_default_logExt}; ///< The extension for the log files.

   size_t m_maxLogSize {XWCTK_default_max_logSize}; ///< The maximum file size in bytes. Default is 10 MB.
   ///@}
...
protected:
   ...
   /** Closes the current file if open.  Then creates a new file with a name of the form
     ...
     * \returns -1 on error
     */
   int createFile(flatlogs::timespecX & ts /**< [in] A XWCTk timespec, used to set the timestamp */);
