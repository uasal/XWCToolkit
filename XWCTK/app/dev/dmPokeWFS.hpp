/** \file dmPokeWFS.hpp
  * \brief The XWCToolkit DM Poke Centering header file
  *
...
/** \defgroup dmPokeWFS
  * \brief The XWCToolkit device to coordinate poking a deformable mirror's actuators and synchronize reads of a camera image.
  *
...
/** CRTP class `derivedT` has the following requirements:
  * 
  * - Must be derived from XWCTkApp<true>
  * 
  ...
  * 
  * - Must contain the following friend declalibXWCTk/app/indiMacros.hpprations (replace DERIVEDNAME with derivedT class name):
  *   \code
