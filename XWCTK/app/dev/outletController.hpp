...
/** Controls a set of outlets on a device, such as A/C power outlets or digital outputs.
  ...
  *
  * derivedT must be a XWCTkApp, and additionally it must implement the functions
  * \code
  ...
  * 
  * 
  * \tparam derivedT specifies a XWCTkApp parent base class which is accessed with a `static_cast` (downcast)
...
struct outletController
{
   ...
   /** This should be called in the `appStartup` function of the derived XWCTkApp.
