/** \file semUtilsDerived.hpp
  * \brief XWCTK app semaphore Utilities to use CRTP derived classes
...
/** An error would be generated by clock_gettime.
  ...
  *  
  */
#define XWCTK_SEM_WAIT_TS_RETVOID_DERIVED( ts, sec, nsec )                                        \
...
/** An error would be generated by clock_gettime
  ...
  *  
  */
#define XWCTK_SEM_WAIT_TS_DERIVED( ts, sec, nsec )                             \
    if(clock_gettime(CLOCK_REALTIME, &ts) < 0)                               \
    {                                                                        \
        derivedT::template log<software_critical>({__FILE__,__LINE__,errno,0,"clock_gettime"}); \
        return -1;                                                           \
...
/**
  ...
  *  
  */
#define XWCTK_SEM_TIMEDWAIT_LOOP_DERIVED( sem, ts )                                            \
...
#define XWCTK_SEM_FLUSH_DERIVED( sem )                                                          \
{                                                                                     \
    int semval;                                                                       \
