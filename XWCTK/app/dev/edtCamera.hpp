...
/// XWCToolkit EDT framegrabber interface
/** Implements an interface to the EDT PDV SDK
  * 
  * The derived class `derivedT` must be a XWCTkApp\<true\>, and must declare this class a friend like so: 
  * \code
  ...
  * \endcode
  * which determines whether or not the EDT config path is relative to the XWCToolkit config path (true) or absolute (false).
  *
...
#define XWCTK_PDV_SERBUFSIZE 512
...
{
   char    buf[XWCTK_PDV_SERBUFSIZE+1];

   // Flush the channel first.
   // This does not indicate errors, so no checks possible.
   pdv_serial_read(m_pdv, buf, XWCTK_PDV_SERBUFSIZE);
   ...
   do
   {
      ret = pdv_serial_read(m_pdv, buf, XWCTK_PDV_SERBUFSIZE);
