/** \file magAOXApp.hpp
  * \brief The basic XWCToolkit Application
...
#ifndef app_XWCTkApp_hpp
#define app_XWCTkApp_hpp
...
/// The base-class for XWCToolkit applications.
/**
  ...
  *
  *
  * \todo implement libXWCTk error handling? (a stack?)
...
class XWCTkApp : public application
{

    friend class XWCTkApp_test;

public:

   ///The log manager type.
   typedef logger::logManager<XWCTkApp<_useINDI>, logFileRaw> logManagerT;

protected:

   std::string XWCTkPath; ///< The base path of the XWCToolkit system.

   std::string m_configName; ///< The name of the configuration file (minus .conf).

   std::string m_configDir; ///< The path to configuration files for XWCTk.
      
   std::string m_configBase; ///< The name of a base config class for this app (minus .conf).

   std::string m_calibDir; ///< The path to calibration files for XWCTk.
   ...
   unsigned long m_loopPause {XWCTK_default_loopPause}; ///< The time in nanoseconds to pause the main loop.  The appLogic() function of the derived class is called every m_loopPause nanoseconds.  Default is 1,000,000,000 ns.  Config with loopPause=X.
...
public:
   ...
   /**
     * Only one XWCTkApp can be instantiated per program.  Hence this c'tor will issue exit(-1)
     ...
     *
     * The git repository status is required to create a XWCTkApp.  Derived classes
     * should include the results of running `gengithead.sh` and pass the defined
     * sha1 and modified flags.
     *
     */
   XWCTkApp( const std::string & git_sha1, ///< [in] The current SHA1 hash of the git repository
              const bool git_modified       ///< [in] Whether or not the repo is modified.
            );

   ~XWCTkApp() noexcept(true);
   ...
   /** Replaces the mx::application defaults with the XWCToolkit config system.
     *
     * This function parses the CL for "-n" or "--name".
     *
     *
     * Do not override this unless you intend to depart from the XWCToolkit standard.
     */
   ...
   /// The basic XWCToolkit configuration setup method.  Should not normally be overridden.
   /** This method sets up the config system with the standard XWCToolkit key=value pairs.
     *
     * Though it is virtual, it should not normally be overridden unless you need
     * to depart from the XWCToolkit standard.
     *
     * Setting up app specific config goes in setupConfig() implemented in the derived class.
     */
   virtual void setupBasicConfig();

   /// The basic XWCToolkit configuration processing method.  Should not normally be overridden.
   /** This method processes the standard XWCToolkit key=value pairs.
     *
     * Though it is virtual, it should not normally be overridden unless you need
     * to depart from the XWCToolkit standard.
     *
...
private:
   ...
   /** \name Signal Handling
     * @{libXWCTk/logger/types/software_log.hpp
     */
private:

   static XWCTkApp * m_self; ///< Static pointer to this (set in constructor).  Used to test whether a a XWCTkApp is already instatiated (a fatal error) and used for getting out of static signal handlers.
   ...
   static void _handlerSigTerm( int signum,        ///< [in] specifies the signal.
                                siginfo_t *siginf, ///< [in] ignored by XWCTkApp
                                void *ucont        ///< [in] ignored by XWCTkApp
                              );
...
protected:
   ...
   class elevatedPrivileges
   {
      ...
      public:
         explicit elevatedPrivileges(XWCTkApp * app)
         {
            m_app = app;
            elevate();
         }
...
protected:
   ...
   /** \name PID Locking
     *
     * Each XWCTkApp has a PID lock file in the system directory.  The app will not
   ...
   /** \name cpusets
     ...
     * the environment variable is not set, the default defined by XWCTK_cpusetPath in paths.hpp is used.
     * This is normally "/opt/XWCTk/cpuset/"
     */
   
   std::string m_cpusetPath {XWCTK_cpusetPath};
...
public:
   ...
   indiDriver<XWCTkApp> * m_indiDriver {nullptr};
...
public:
   ...
   int createStandardIndiNumber( pcf::IndiProperty & prop,       ///< [out] the property to create and setup
                                 ...
                                 const std::string & format,     ///< [in] the _ value for the elements, applied to both target and current.  Set to "" to use the XWCToolkit standard for type.
                                 const std::string & label = "", ///< [in] [optional] the GUI label suggestion for this property
                                 const std::string & group = ""  ///< [in] [optional] the group for this property
                               );
...
protected:
  ...
  /// Update an INDI property if values have changed.
   ...
   /**
     ...
     * \returns -1 on error.
     */
   static int st_newCallBack_clearFSMAlert( void * app, ///< [in] a pointer to this, will be static_cast-ed to XWCTkApp.
...
public:
   ...
   INDI_SETCALLBACK_DECL(XWCTkApp, m_indiP_powerChannel);
...
template<bool _useINDI> XWCTkApp<_useINDI> * XWCTkApp<_useINDI>::m_self = nullptr;

//Define the logger
template<bool _useINDI> typename XWCTkApp<_useINDI>::logManagerT XWCTkApp<_useINDI>::m_log;

template<bool _useINDI>
XWCTkApp<_useINDI>::XWCTkApp( const std::string & git_sha1,
                                const bool git_modified
                              )
{
   if( m_self != nullptr )
   {
      std::cerr << "Attempt to instantiate 2nd XWCTkApp.  Exiting immediately.\n";
      exit(-1);
   }
   ...
   log<git_state>(git_state::messageT("XWCTk", git_sha1, git_modified), gl);
...
template<bool _useINDI>
XWCTkApp<_useINDI>::~XWCTkApp() noexcept(true)
{
   if(m_indiDriver) delete m_indiDriver;
   m_log.parent(nullptr);

   XWCTkApp<_useINDI>::m_self = nullptr;
}

template<bool _useINDI>
int XWCTkApp<_useINDI>::shutdown()
{
   return m_shutdown;
}

template<bool _useINDI>
void XWCTkApp<_useINDI>::setDefaults( int argc,
                                       char ** argv
                                     )   //virtual
{
   std::string tmpstr;

   tmpstr = mx::sys::getEnv(XWCTK_env_path);
   if(tmpstr != "")
   {
      XWCTkPath = tmpstr;
   }
   ...
   {
      XWCTkPath = XWCTK_path;
   }

   //Set the config path relative to XWCTkPath
   tmpstr = mx::sys::getEnv(XWCTK_env_config);
   if(tmpstr == "")
   {
      tmpstr = XWCTK_configRelPath;
   }
   m_configDir = XWCTkPath + "/" + tmpstr;
   m_configPathGlobal = m_configDir + "/xwctk.conf";

   //Set the calib path relative to XWCTkPath
   tmpstr = mx::sys::getEnv(XWCTK_env_calib);
   if(tmpstr == "")
   {
      tmpstr = XWCTK_calibRelPath;
   }
   m_calibDir = XWCTkPath + "/" + tmpstr;
   
   
   
   //Setup default log path
   tmpstr = XWCTkPath + "/" + XWCTK_logRelPath;

   m_log.logPath(tmpstr);

   //Setup default sys path
   tmpstr = XWCTkPath + "/" + XWCTK_sysRelPath;
   sysPath = tmpstr;

   //Setup default secrets path
   tmpstr = XWCTkPath + "/" + XWCTK_secretsRelPath;
   secretsPath = tmpstr;

   //Setup default cpuset path
   tmpstr = mx::sys::getEnv(XWCTK_env_cpuset);
   if(tmpstr != "")
   {
      m_cpusetPath = tmpstr;
   }
...
template<bool _useINDI>
void XWCTkApp<_useINDI>::setupBasicConfig() //virtual
{
   //App stuff
...
template<bool _useINDI>
void XWCTkApp<_useINDI>::loadBasicConfig() //virtual
{
   bool ig {false};
...
template<bool _useINDI>
void XWCTkApp<_useINDI>::checkConfig() //virtual
{
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::execute() //virtual
{
   //----------------------------------------//
   //        Check user
   //----------------------------------------//
...
int XWCTkApp<_useINDI>::log( const typename logT::messageT & msg,
                              logPrioT level
                            )
{
...
template<bool _useINDI>
template<typename logT, int retval>
int XWCTkApp<_useINDI>::log( logPrioT level)
{
...
template<bool _useINDI>
void XWCTkApp<_useINDI>::logMessage( bufferPtrT & b )
{
...
template<bool _useINDI>
void XWCTkApp<_useINDI>::configLog( const std::string & name,
                                     const int & code,
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::setSigTermHandler()
{
   struct sigaction act;
   sigset_t set;

   act.sa_sigaction = &XWCTkApp<_useINDI>::_handlerSigTerm;
...
template<bool _useINDI>
void XWCTkApp<_useINDI>::_handlerSigTerm( int signum,
                                           siginfo_t *siginf,
                                           void *ucont
                                         )
{
...
template<bool _useINDI>
void XWCTkApp<_useINDI>::handlerSigTerm( int signum,
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::setEuidCalled()
{
   errno = 0;
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::setEuidReal()
{
   errno = 0;
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::unlockPID()
{
   {//scope for elPriv
...
template<bool _useINDI>
template<class thisPtr, class Function>
int XWCTkApp<_useINDI>::threadStart( std::thread & thrd,
                                      bool & thrdInit,
                                      pid_t & tpid,
                                    ...
                                    )
{
   ...
   {//scope for elPriv
      ...
      errno = 0;
      if(thrdPrio > 0) rv = pthread_setschedparam(thrd.native_handle(), XWCTK_RT_SCHED_POLICY, &sp);
...
template<bool _useINDI>
stateCodes::stateCodeT XWCTkApp<_useINDI>::state()
{
   return m_state;
}

template<bool _useINDI>
void XWCTkApp<_useINDI>::state( const stateCodes::stateCodeT & s,
                                 bool stateAlert
                               )
{
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::stateLogged()
{
   if(m_stateLogged > 0)
   {
      ++m_stateLogged;
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::clearFSMAlert()
{
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::createStandardIndiText( pcf::IndiProperty & prop,       
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::createROIndiText( pcf::IndiProperty & prop,           
                                           const std::string & propName,       
...
template<bool _useINDI>
template<typename T>
int XWCTkApp<_useINDI>::createStandardIndiNumber( pcf::IndiProperty & prop,       
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::createROIndiNumber( pcf::IndiProperty & prop,           
                                             const std::string & propName,       
                                             const std::string & propLabel, 
                                             const std::string & propGroup 
                                           )
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::createStandardIndiToggleSw( pcf::IndiProperty & prop,       
                                                     const std::string & name,       
                                                     const std::string & label, 
                                                     const std::string & group  
                                                   )
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::createStandardIndiRequestSw( pcf::IndiProperty & prop,   
                                                      const std::string & name,   
                                                      const std::string & label,  
                                                      const std::string & group   
                                                    )
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::createStandardIndiSelectionSw( pcf::IndiProperty & prop,                  
                                                        const std::string & name,                  
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::registerIndiPropertyReadOnly( pcf::IndiProperty & prop )
{
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::registerIndiPropertyNew( pcf::IndiProperty & prop,
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::registerIndiPropertySet( pcf::IndiProperty & prop,
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::createINDIFIFOS()
{
   if(!m_useINDI) return 0;

   ///\todo make driver FIFO path full configurable.
   std::string driverFIFOPath = XWCTK_path;
   driverFIFOPath += "/";
   driverFIFOPath += XWCTK_driverFIFORelPath;
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::startINDI()
{
   if(!m_useINDI) return 0;
   ...
   try
   {
      ...
      m_indiDriver = new indiDriver<XWCTkApp>(this, m_configName, "0", "0");
   }
   catch(...)
   {
...
template<bool _useINDI>
void XWCTkApp<_useINDI>::sendGetPropertySetList(bool all)
{
...
template<bool _useINDI>
void XWCTkApp<_useINDI>::handleDefProperty( const pcf::IndiProperty &ipRecv )
{
   handleSetProperty(ipRecv); //We have the same response to both Def and Set.
}

template<bool _useINDI>
void XWCTkApp<_useINDI>::handleGetProperties( const pcf::IndiProperty &ipRecv )
{
    if(!m_useINDI) return;
...
template<bool _useINDI>
void XWCTkApp<_useINDI>::handleNewProperty( const pcf::IndiProperty &ipRecv )
{
   if(!m_useINDI) return;
   if(m_indiDriver == nullptr) return;
...
template<bool _useINDI>
void XWCTkApp<_useINDI>::handleSetProperty( const pcf::IndiProperty &ipRecv )
{
   if(!m_useINDI) return;
   if(m_indiDriver == nullptr) return;
...
template<bool _useINDI>
template<typename T>
void XWCTkApp<_useINDI>::updateIfChanged( pcf::IndiProperty & p,
                                           const std::string & el,
                                           const T & newVal, 
...
template<bool _useINDI>
void XWCTkApp<_useINDI>::updateSwitchIfChanged( pcf::IndiProperty & p,
                                                 const std::string & el,
...
template<bool _useINDI>
template<typename T>
int XWCTkApp<_useINDI>::indiTargetUpdate( pcf::IndiProperty & localProperty,
                                           T & localTarget,
...
template<bool _useINDI>
template<typename T>
int XWCTkApp<_useINDI>::sendNewProperty( const pcf::IndiProperty & ipSend,
                                          const std::string & el,
                                          const T & newVal
                                        )
{
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::sendNewStandardIndiToggle( const std::string & device,
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::st_newCallBack_clearFSMAlert( void * app,
                                                       const pcf::IndiProperty &ipRecv
                                                     )
{
   return static_cast<XWCTkApp<_useINDI> *>(app)->newCallBack_clearFSMAlert(ipRecv);
}

template<bool _useINDI>
int XWCTkApp<_useINDI>::newCallBack_clearFSMAlert( const pcf::IndiProperty &ipRecv )
{
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::onPowerOff() 
{
   return 0;
}

template<bool _useINDI>
int XWCTkApp<_useINDI>::whilePowerOff() 
{
   return 0;
}

template<bool _useINDI>
bool XWCTkApp<_useINDI>::powerOnWaitElapsed()
{
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::powerState()
{
...
template<bool _useINDI>
int XWCTkApp<_useINDI>::powerStateTarget()
{
   if(!m_powerMgtEnabled) return 1;
...
template<bool _useINDI>
INDI_SETCALLBACK_DEFN( XWCTkApp<_useINDI>, m_indiP_powerChannel)(const pcf::IndiProperty &ipRecv)
{
   std::string ps;
...
template<bool _useINDI>
std::string XWCTkApp<_useINDI>::configName()
{
   return m_configName;
}

template<bool _useINDI>
std::string XWCTkApp<_useINDI>::configDir()
{
   return m_configDir;
}

template<bool _useINDI>
std::string XWCTkApp<_useINDI>::driverInName()
{
   return m_driverInName;
}

template<bool _useINDI>
std::string XWCTkApp<_useINDI>::driverOutName()
{
   return m_driverOutName;
}

template<bool _useINDI>
std::string XWCTkApp<_useINDI>::driverCtrlName()
{
   return m_driverCtrlName;
}

extern template class XWCTkApp<true>;
extern template class XWCTkApp<false>;

} //namespace app
} //namespace XWCTk

#endif //app_XWCTkApp_hpp
